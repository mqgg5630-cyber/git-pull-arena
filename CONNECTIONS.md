...
|---|---|---|---|---|---|
| **git-sync 技能总部**（当前会话 `arena.ai/agent/01a0a98d-…`） | [mqgg5630-cyber/git-pull-arena](https://github.com/mqgg5630-cyber/git-pull-arena) | **`arena/01a0a98d-git-pull-arena`**（v2.7.4；上一会话分支 `arena/01a0a821-git-pull-arena` 已冻结在 `6c25969`） | `E:\0github\git-sync\git-pull-arena-s2`（同一个克隆，切分支即可） | `git-sync-watch-git-pull-arena-s2` | ⚠️ **round 19 从 07:38 UTC 起一直 pending**（本机最后一次推结论是 03:03 UTC 的 round 18）；回到本机后 `.\sync.ps1` + `.\watch.ps1 -Focus` 即接上 |
| 中期报告自循环（会话 `arena.ai/agent/01a0a95e-…`） | [mqgg5630-cyber/zhongqi](https://github.com/mqgg5630-cyber/zhongqi) | `arena/01a0a95e-zhongqi`（tip `c88dafe`） | 本机已接管（`LAPTOP-R77M5D6M`） | 已注册并回传 | ✅ **round 1 passed**（本机 09:33:55Z 推回 `check: round 1 passed`；17 个产物 sha256/体积/OOXML 全对）。唯一尾巴：`arena_state` 仍是 `awaiting_check`——那个会话的沙箱已消失，没人执行 `agent-check.sh --accept` |
...
- 2026-09-16：**v2.7.5（把"手打交接命令"这个根因堵死）**：三个真实事故——`01a0a95e` 的 round 1 请求在 `pending` 挂了 2 小时（本机根本没有那条分支的克隆/值守）、本会话 `01a0a98d` 的 `sync.config.json` 分支停在上个会话（`agent-sync.sh` 直接 `[REFUSED]`）、以及把沙箱路径 `/home/user/...` 写进 Windows PowerShell 块——全部来自手打。本版：① 新增 `skills/git-sync/scripts/agent-handoff.sh`：从 `git remote get-url` + 配置 branch **生成**可粘的 PowerShell（`--json` 输出真值），配置分支 ≠ HEAD 时 exit 3 拒绝；② `code/check_all.sh` 新增 2b 段「配置 branch 必须 == HEAD」，不一致 exit 1（值守轮询的是配置分支）；③ `agent-check.sh --read` 报告 pending 了多少分钟，≥10 分钟提示重发交接块而不是继续空轮询；④ `templates/one-sentence.md` 铁律 3 改为"那段由生成器产出，不得手打"。`VERSION` 2.7.4→2.7.5，`success_criteria.json` 同步（20 ok / 0 fail）。同日本机把 zhongqi `01a0a95e` round 1 判了 passed（`c88dafe`）。
- 2026-09-16：**为什么"没先与本机同步"（`01a0a95e` 会话 round 1 无人应答的根因，全部有远端证据）**：① 产物推到了 **zhongqi 仓库** 的 `arena/01a0a95e-zhongqi`，而本机 `E:\0zhongqi\zhongqi` 在下面台账里就是**值守"未注册"**，且它停在 `arena/01a09d79-zhongqi`（那个分支只有 `bootstrap.ps1`，**没有 `watch.ps1`**；v2.7.4 的 `watch.ps1` 只在 `01a0a949`/`01a0a95e` 上）；② 在 `01a0a95e` 克隆里跑 `watch.ps1 -Register` 会走 `Invoke-ParkOthers`——**把本机其它 `git-sync-watch-*` 任务全部 Stop+Disable**，包括正在用的 `git-sync-watch-git-pull-arena-s2`（恢复：`.\watch.ps1 -RestoreParked`）；③ 本仓库 `skills/git-sync/sync.config.json` 的 `branch` 还停在上一会话的 `arena/01a0a821-git-pull-arena`，而本会话 HEAD 是 `arena/01a0a98d-git-pull-arena` → `agent-sync.sh` 直接 `[REFUSED] HEAD is on …, expected …`，**本轮已改成 `arena/01a0a98d-git-pull-arena`**；④ 沙箱后台那条 `agent-handsfree --timeout 1800` 随会话结束就消失，不能跨会话等人；⑤ 结论：agent 侧断言/闸门全绿是真的（本轮 `agent-criteria.sh` 17/17 PASSED、`code/check_all.sh` exit 0），但**本机回执一个都没到**（`handshake.json`：`local_state=pending`、`local_updated=null`），按技能自己的判据**不算打通**。
