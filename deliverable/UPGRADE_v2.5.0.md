...
> **先看 [`FIX_v2.5.1.md`](FIX_v2.5.1.md)**：v2.5.0 实测在你机器上撞到两个 bug
> （`watch.ps1` 的 `"$round:"` 导致整份脚本解析失败；`auth.ps1 -Setup` 把
> `credentialStore` 改成 dpapi 反而藏掉了原有凭据）。修复版 v2.5.1 已发布，
> 本机照 `FIX_v2.5.1.md` 第一节做 5 条命令即可。
